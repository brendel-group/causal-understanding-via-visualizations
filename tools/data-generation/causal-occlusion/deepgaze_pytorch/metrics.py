...
def log_likelihood(log_density, fixation_mask, weights=None):
    ...
    ll = torch.mean(
        weights
        * torch.sum(log_density * dense_mask, dim=(-1, -2), keepdim=True)
        / fixation_count
    )
...
def nss(log_density, fixation_mask, weights=None):
    ...
    nss = torch.mean(
        weights
        * torch.sum(saliency_map * dense_mask, dim=(-1, -2), keepdim=True)
        / fixation_count
    )
    return nss
...
def auc(log_density, fixation_mask, weights=None):
    ...
    def image_auc(log_density, fixation_mask):
        dense_mask = fixation_mask.to_dense()
        positives = (
            torch.masked_select(log_density, dense_mask.type(torch.bool))
            .detach()
            .cpu()
            .numpy()
        )
